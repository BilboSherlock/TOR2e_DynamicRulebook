...
import { EriadorMap } from './components/EriadorMap';
import { SearchModal } from './components/SearchModal';
import { StyleGuideModal } from './components/StyleGuideModal';
import { LoremasterAuthModal } from './components/LoremasterAuthModal';
...
export default function App() {
  ...
  // Primary Map selection state
  const [activePrimaryMap, setActivePrimaryMap] = useState<'eriador' | 'shire'>(
    MAP_GLOBAL_CONFIG.defaultPrimaryMap
  );
  ...
  const [isLoreMasterAuthOpen, setIsLoreMasterAuthOpen] = useState(false);

  // Filtered chapter collections
  const coreRulesChapters = useMemo(() => chapters.filter((ch) => ch.supplement === 'Core Rules' || ch.supplement === 'Starter Set'), [chapters]);
  ...
  const handleToggleLoreMasterMode = useCallback(() => {
    if (!isLoreMasterMode) {
      // Require riddle confirmation to turn ON
      setIsLoreMasterAuthOpen(true);
    } else {
      // Turning OFF directly
      setIsLoreMasterMode(false);
      if (isLoreMasterChapter(activeChapter)) {
  ...
  return (
    <div className={`flex flex-col bg-[#EFE8D3] text-[#28211D] font-serif transition-colors ${
      activeView === 'toc' ? 'h-screen h-dvh max-h-screen overflow-hidden' : 'min-h-screen'
    }`}>
      {/* Header Bar - Hidden on TOC page to maximize Table of Contents viewport area */}
      {activeView !== 'toc' && (
        <Header
          ...
          activePrimaryMap={activePrimaryMap}
          onSwitchPrimaryMap={setActivePrimaryMap}
        />
      )}

      {/* Main Layout Area */}
      <div className={`flex-1 max-w-7xl w-full mx-auto flex items-start ${
        activeView === 'toc' ? 'h-full min-h-0 overflow-hidden' : ''
      }`}>
        {/* Navigation Sidebar (Hidden on Map & TOC views for maximum focus and space) */}
        {activeView !== 'map' && activeView !== 'toc' && (
          <Sidebar
            chapters={chapters}
        ...
        {/* Dynamic Main View Area - High Performance Native View Transition */}
        <div className={`flex-1 min-w-0 w-full transition-opacity duration-150 ease-out ${
          activeView === 'toc' ? 'h-full min-h-0 overflow-hidden' : ''
        }`}>
          {activeView === 'toc' && (
            <TableOfContents
          ...
          {activeView === 'map' && (
            <EriadorMap
              ...
              onSelectHeroicCulture={handleSelectCulturePage}
              activePrimaryMap={activePrimaryMap}
              onSwitchPrimaryMap={setActivePrimaryMap}
            />
          )}
